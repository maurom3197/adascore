...
class ActorCNNetwork(Model):
	def __init__(self, max_action, height = 112, width  = 112,  lr = 0.00025, conv_units=(32,64),
			filt_size = (3,3), fc_units = (128,128),  name = 'actor'):
		...
		self.image_input = Input(shape=(self.height, self.width, self.channels,))  
		self.image_shape = (1, self.height, self.width, self.channels,)
		
		#Hidden Layer
		self.k_initializer = HeUniform()
		self.conv1 = Conv2D(self.conv_units[0], filt_size[0], strides=(2, 2), activation='relu', kernel_initializer = self.k_initializer)
		self.conv1_1 = Conv2D(self.conv_units[0], filt_size[0], strides=(1, 1), activation='relu', kernel_initializer = self.k_initializer)
		self.max_pool = MaxPooling2D(pool_size=(2, 2), strides=(2,2))
		self.conv2 = Conv2D(self.conv_units[1], filt_size[1], strides=(2, 2), activation='relu', kernel_initializer = self.k_initializer)
		self.conv3 = Conv2D(self.conv_units[1], filt_size[1], strides=(1, 1), activation='relu', kernel_initializer = self.k_initializer)

		self.fc1 = Dense(fc_dims[0], activation='relu', kernel_initializer = self.k_initializer)
		self.fc2 = Dense(fc_dims[1], activation='relu', kernel_initializer = self.k_initializer)
		#Output Layer
		self.linear_out = Dense(1, activation = 'sigmoid')
		self.angular_out = Dense(1, activation = 'tanh')
...
class CriticCNNetwork(Model):
	def __init__(self, height = 112, width = 112, lr = 0.001,  unitsc1 = 32, unitsc2 = 32, filt1_size = 3, filt2_size = 3,
