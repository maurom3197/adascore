...
from .utils.generic_sensor import Sensors
from .utils.env_utils import *
from .utils.sfm import SocialForceModel
...
class Pic4rlEnvironmentAPPLR(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ("use_localization", main_params["use_localization"]),
            ],
        )
        ...
        self.gazebo_client = (
            self.get_parameter("gazebo_client").get_parameter_value().bool_value
        )
        # -- Noé --
        ...
        self.robot_desc = (
            self.call_get_parameters("robot_state_publisher", {"robot_description"})
            .values[0]
            .string_value
        )
        self.get_logger().warn("Robot name: %s" % self.robot_name)
        self.get_logger().warn("\n\nROBOT_DESCRIPTION:\n%s\n\n" % self.robot_desc)

        # create log dir
        self.logdir = create_logdir(
            training_params["--policy"],
            main_params["sensor"],
        ...
        #         qos
        #     )
        self.cli = self.create_client(SetEntityState, "test/set_entity_state")
        while not self.cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "service /test/set_entity_state not available, waiting again..."
            )
        ...
        self.max_person_dist_allowed = 6.0

        self.initial_pose, self.goals, self.poses, self.agents = get_goals_and_poses(
            self.data_path
        )
        ...
        # 7 params:
        # cov front height = 0.4  REPLACED BY
        # speed factor multiplier = 5.0
        # cov front width = 0.25
        # cov static = 0.25
        # max vel x = 0.8 REPLACED BY path align scale
        # obstacle scale = 0.02
        # path dist scale = 32.0
        # goal dist scale = 24.0
        # path align scale = 32.0
        self.init_nav_params = [5.0, 0.25, 0.25, 0.1, 32.0, 24.0, 32.0]
    ...
    def call_get_parameters(self, node_name, parameter_names):
        # create client
        ...
        if response is None:
            e = future.exception()
            raise RuntimeError("Exception while calling get_parameters")
        return response
    ...
    def _step(self, nav_params=None, reset_step=False):
        ...
        if done:
            ...
            req.command = ManageLifecycleNodes.Request.PAUSE  # 1
            fut = self.set_cli_lifecyle.call_async(req)
    ...
    def send_action(self, params):
        """ """
        costmap_params = params[:-4]  # first 3 params
        controller_params = params[-4:]  # last 4 params
    ...
    def check_events(self, lidar_measurements, goal_info, robot_pose, collision):
        ...
        # self.get_logger().info('Navigator feedback: '+str(feedback))
        ...
        if self.navigator.isTaskComplete():
            ...
            if result == TaskResult.FAILED or result == TaskResult.CANCELED:
                # --Noé ----
                # When the navresult is failed I get these possible errors in the costmap:
                # Message Filter dropping message: frame 'mid_laser_mount' at time 676.104 for reason 'Unknown'
                # This error hapens and the robot's TFs do not move to the correct place.
                # Clearing costmap -> not solved.
                # respawn robot -> not solved.
                # reset lifecycle nodes??? -> not solved.
                # all together -> not solved.
                # count the number of fails and move to othe episode?
                # self.navigator.cancelTask()
                # self.get_logger().debug("Nav Failed. Respawning2 robot in Gazebo...")
                # self.respawn_robot2(self.index)
                # self.get_logger().debug("Resetting navigator ...")
                # self.reset_navigator(self.index) #clear costamps, waituntinav, and sendgoal
                # time.sleep(1.0)

                # self.n_navigation_end = self.n_navigation_end +1
                # if self.n_navigation_end == 10: #25: #50:

                self.get_logger().info(
                    "Navigation aborted, pausing Nav till next episode."
                )
                self.get_logger().info(
                    f"Ep {'evaluate' if self.evaluate else self.episode+1}: 'Nav2 failed'"
                )
                logging.info(
                    f"Ep {'evaluate' if self.evaluate else self.episode+1}: Nav2 failed"
                )
                self.prev_nav_state = "nav2_failed"
                # self.n_navigation_end = 0 #--Noé
            ...
            if result == TaskResult.SUCCEEDED:
                if self.prev_nav_state == "goal":
                    self.get_logger().info(
                        "uncorrect goal status detected... resending goal."
                    )
                    self.get_logger().info(
                        "CheckEvents. Sending goal. sx: "
                        + str(robot_pose[0])
                        + " sy: "
                        + str(robot_pose[1])
                        + " gx: "
                        + str(self.goal_pose[0])
                        + " gy:"
                        + str(self.goal_pose[1])
                    )
                    ok = self.send_goal(robot_pose, self.goal_pose)
                    # --Noé
                    if not ok:
                        self.navigator.cancelTask()
                        self.get_logger().debug("Respawning robot in Gazebo...")
    ...
    def reward2(
        ...
        self.get_logger().debug("People proxemics reward Rp: " + str(cp * Rp))
        self.get_logger().debug("Social work reward Rs: " + str(cs * Rs))
        self.get_logger().debug("step reward: " + str(ce * Re))
        reward = ce * Re + cs * Rs + cp * Rp
        self.get_logger().debug("total reward: " + str(reward))
    ...
    def reset(self, n_episode):
        ...
        #     stdout=subprocess.DEVNULL,
        # )
        self.get_logger().info("reset. Resetting managed nodes...")
        req = ManageLifecycleNodes.Request()
        req.command = ManageLifecycleNodes.Request.RESET  # PAUSE  # 1
        fut = self.set_cli_lifecyle.call_async(req)
        rclpy.spin_until_future_complete(self, fut, timeout_sec=2.0)
        time.sleep(3.0)
    ...
    def reset(self, n_episode, tot_steps, evaluate=True):
        """ """
        self.episode = n_episode
        self.evaluate = evaluate

        # self.get_logger().debug("pausing...")
        # self.pause() # --Noé. commented

        logging.info(
            f"Total_episodes: {'evaluate' if evaluate else n_episode}, Total_steps: {tot_steps}, episode_steps: {self.episode_step+1}\n"
        )

        # if self.failure_counter == 4:
        #     self.get_logger().info(
        #         "reset. failure_counter = 4! Restarting gazebo simulation and nav2..."
        #     )
    ...
    def new_episode(self):
        ...
        end_time = time.time()
        # elapsed seconds
        elapsed_time = end_time - self.inittime
        mins = elapsed_time / 60.0
        hours = mins / 60.0
        print("\nElapsed time - total hours: %.2f, total min: %.2f" % (hours, mins))
        ...
        if self.episode % self.change_episode == 0.0:
        ...
        self.get_logger().info("New episode. Respawning1 robot in Gazebo...")
        self.respawn_robot(self.index)

        if self.episode % 10 == 0.0:
            self.get_logger().info("Episode mod of 10. Respawning all agents ...")
            self.respawn_agents()  # all=True)
        # else:
        #     self.respawn_agents()

        # self.get_logger().info("Updating goal index ...")
    ...
    def respawn_robot(self, index):
        ...
        req.state.pose.position.z = 0.07
        ...
        get_response = self.future.result()
        self.get_logger().info(
            "Respawn_robot. SetEntityState result: %s" % str(get_response)
        )
        ...
        # cmd = "ros2 service call /test/set_entity_state gazebo_msgs/srv/SetEntityState "+state
        ...
        # time.sleep(1.0)
        # -- Noé --
        # set the amcl position
        if self.use_localization:
            p = PoseStamped()  # PoseWithCovarianceStamped()
            p.header.frame_id = "map"
            t = self.get_clock().now()
            p.header.stamp = t.to_msg()
            p.pose.position.x = x
            p.pose.position.y = y
            p.pose.orientation.z = qz
            p.pose.orientation.w = qw
            # self.pos_pub.publish(p)
            self.navigator.setInitialPose(p)

        time.sleep(1.0)

    def respawn_robot2(self, index):
        x, y, yaw = tuple(self.poses[index])

        qz = np.sin(yaw / 2)
        qw = np.cos(yaw / 2)

        self.get_logger().info("Respawn_robot2")
        self.get_logger().info(
            f"Respawn_robot2. Ep {'evaluate' if self.evaluate else self.episode+1} robot pose [x,y,yaw]: {[x, y, yaw]}"
        )
        logging.info(
            f"Ep {'evaluate' if self.evaluate else self.episode+1} robot pose [x,y,yaw]: {[x, y, yaw]}"
        )
        ...
        # ERRORS:
        # sometimes delete or spawn fail, and the exception is not triggered.
        # get_response = None.
        # We try to call it again but it is not working.
        # respawn_robot2 works ok in all cases except the one needed,
        # when the nav fails.

        # self.pause()
        # self.get_logger().info("Respawn_robot2. After pausing Gazebo")

        # First delete robot
        req = DeleteEntity.Request()
        req.name = self.robot_name
        try:
            future = self.delete_robot_client.call_async(req)
            rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
            get_response = future.result()
        except Exception as e:
            self.get_logger().info("Service call failed: %r" % (e,))
        self.get_logger().info(
            "Respawn_robot2. After deleting entity robot. Response: %s"
            % str(get_response)
        )
        if get_response == None:
            self.get_logger().info(
                "Respawn_robot2. Response delete = None. Calling again!"
            )
            try:
                future = self.delete_robot_client.call_async(req)
                rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
                ...
                self.get_logger().info("Service call failed2: %r" % (e,))
            self.get_logger().info(
                "Respawn_robot2. After deleting entity robot 2. Response: %s"
                % str(get_response)
            )
        ...
        req.initial_pose.position.z = 0.07
        req.initial_pose.orientation.z = qz
        req.initial_pose.orientation.w = qw
        req.xml = self.robot_desc
        self.get_logger().info("Respawn_robot2. Spawn request: %s" % str(req))
        try:
        ...
        self.get_logger().info(
            "Respawn_robot2. After respawning entity robot. Response: %s"
            % str(get_response)
        )
        while get_response == None:
            self.get_logger().info(
                "Respawn_robot2. Response spawn = None. Calling again!"
            )
            self.destroy_client(self.spawn_robot_client)
            ...
            while not self.spawn_robot_client.wait_for_service(timeout_sec=1.0):
                self.get_logger().info(
                    "Spawn robot client get service not available, waiting again..."
                )
            try:
                future = self.spawn_robot_client.call_async(req)
                rclpy.spin_until_future_complete(self, future, timeout_sec=1.0)
                ...
                self.get_logger().info("Service call failed2 %r" % (e,))
            self.get_logger().info(
                "Respawn_robot2. After respawning entity robot2. Response: %s"
                % str(get_response)
            )

        # time.sleep(1.0)
        self.unpause()
        # self.get_logger().info("Respawn_robot2. After unpausing Gazebo")
    ...
    def respawn_agents(self, all=False):
        ...
        for agent in range(1, len(self.agents)):
            ...
            self.get_logger().info("Respawning Agent: " + agent_name)
            self.get_logger().info(f"Respawning Agent at pose [x,y,yaw]: {[x, y, yaw]}")

            # position = (
            #     "position: {x: " + str(x) + ",y: " + str(y) + ",z: " + str(1.50) + "}"
            # )
            # # orientation = "orientation: {z: "+str(qz)+",w: "+str(qw)+"}"
            # # pose = position+", "+orientation
            ...
            req = SetEntityState.Request()
            req.state.name = agent_name
            req.state.pose.position.x = x
            req.state.pose.position.y = y
            req.state.pose.position.z = 1.50
            req.state.pose.orientation.z = qz
            req.state.pose.orientation.w = qw
            req.state.twist.angular.x = 0.0
            req.state.twist.angular.y = 0.0
    ...
    def reset_navigator(self, index):
        ...
        init_pose.header.frame_id = "map"  #'odom'
        ...
        # if not self.simulation_restarted == 1:  # if not
        # self.get_logger().info("reset_navigator. Restarting LifeCycleNodes...")
        # subprocess.run(
        ...
        # )
        req = ManageLifecycleNodes.Request()
        req.command = ManageLifecycleNodes.Request.RESUME  # 2
        fut = self.set_cli_lifecyle.call_async(req)
        rclpy.spin_until_future_complete(self, fut, timeout_sec=2.0)
        time.sleep(1.0)
        self.n_navigation_end = 0

        # self.navigator.setInitialPose(init_pose)  # --Noé

        # self.get_logger().debug("Clearing all costmaps...")
        self.get_logger().info("Clearing all costmaps...")
        self.navigator.clearAllCostmaps()
        time.sleep(1.5)
        # self.get_logger().debug("wait until Nav2Active...")
        self.get_logger().info("wait until Nav2Active...")
        self.navigator.waitUntilNav2Active(navigator="bt_navigator", localizer="")

        # self.get_logger().debug("Sending goal ...")
        self.get_logger().info(
            "reset_navigator. Sending goal. sx: "
            + str(x)
            + " sy:"
            + str(y)
            + " gx:"
        ...
        self.send_goal([x, y, yaw], self.goal_pose)
        # time.sleep(1.0)
    ...
    def send_set_request_global(self, param_values):
        self.set_req_global.parameters = [
            ...
            Parameter(
                name="social_layer.speed_factor_multiplier", value=param_values[0]
            ).to_parameter_msg(),
            Parameter(
                name="social_layer.covariance_front_width", value=param_values[1]
            ).to_parameter_msg(),
            Parameter(
                name="social_layer.covariance_rear_width", value=param_values[1]
            ).to_parameter_msg(),
            Parameter(
                name="social_layer.covariance_when_still", value=param_values[2]
            ).to_parameter_msg(),
    ...
    def send_set_request_controller(self, param_values):
        self.set_req_controller.parameters = [  # Parameter(name='FollowPath.max_vel_x', value=param_values[0]).to_parameter_msg(),
            ...
            Parameter(
                name="FollowPath.BaseObstacle.scale", value=param_values[0]
            ).to_parameter_msg(),
            Parameter(
                name="FollowPath.PathDist.scale", value=param_values[1]
            ).to_parameter_msg(),
            Parameter(
                name="FollowPath.GoalDist.scale", value=param_values[2]
            ).to_parameter_msg(),
            Parameter(
                name="FollowPath.PathAlign.scale", value=param_values[3]
            ).to_parameter_msg(),
        ]
    ...
    def send_goal(self, start, goal):
        # Set the robot's goal pose
        goal_pose = PoseStamped()
        goal_pose.header.frame_id = (
            "map"  #'odom' is valid too since they are static in the same place
        )
        t = self.get_clock().now()
        ...
        init_pose = PoseStamped()
        init_pose.header.stamp = goal_pose.header.stamp
        init_pose.pose.position.x = start[0]
        init_pose.pose.position.y = start[1]
        qz = np.sin(start[2] / 2)
        qw = np.cos(start[2] / 2)
        ...
        #     self.get_logger().error("Error calling action goToPose!!")
        #     return False

        # getPath(start, goal, planner_id="", use_start=False)
        path = self.navigator.getPath(start=init_pose, goal=goal_pose)
        if path is not None:
            # smoothed_path = self.navigator.smoothPath(path)
            ok = self.navigator.followPath(
                path, controller_id="FollowPath", goal_checker_id="goal_checker"
            )
            if not ok:
                self.get_logger().error("Enviroment-sendGoal. Error following the path")
                return False
        else:
            self.get_logger().error("Enviroment-sendGoal. Error computing path")
            return False
        return True

    # --Noé - change this function
    # get the global costmap and take random samples
    # inside of it with cost equals to zero
    ...
    def pause(self):
        ...
        while not self.pause_physics_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "Gazebo pause service not available, waiting again..."
            )
        ...
        try:
            get_response = future.result()
            # self.get_logger().debug(
            #    '/pause_physics. Result %s' %
            #    (get_response.results[0].successful))
        except Exception as e:
            self.get_logger().debug("Service /pause_physics call failed %r" % (e,))

    def unpause(self):
        req = Empty.Request()
        while not self.unpause_physics_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "Gazebo unpause service not available, waiting again..."
            )
        ...
        try:
            get_response = future.result()
            # self.get_logger().debug(
            #    '/unpause_physics. Result %s' %
            #    (get_response.results[0].successful))
        except Exception as e:
            self.get_logger().debug("Service /unpause_physics call failed %r" % (e,))
    ...
    def create_clients(
        ...
        while not self.get_cli_controller.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "Controller server get service not available, waiting again..."
            )
        ...
        while not self.set_cli_controller.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "Controller server set service not available, waiting again..."
            )
        self.set_req_controller = SetParameters.Request()

        self.set_cli_lifecyle = self.create_client(
            ManageLifecycleNodes, "/lifecycle_manager_navigation/manage_nodes"
        )
        while not self.set_cli_lifecyle.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "lifecycle manager server not available, waiting again..."
            )
        # ros2 service call /lifecycle_manager_navigation/manage_nodes nav2_msgs/srv/ManageLifecycleNodes '{command: 2}'",
        ...
        # --Noe
        self.spawn_robot_client = self.create_client(SpawnEntity, "spawn_entity")
        self.delete_robot_client = self.create_client(DeleteEntity, "delete_entity")
