...
class Pic4rlTraining_APPLR_social_nav(Pic4rlEnvironmentAPPLR):
    def __init__(self):
        super().__init__()
    ...
    def instantiate_agent(self):
        ...
        """
        action=[
        #[0.15, 2.0], # covariance front height
        [0.5, 10.0], # speed factor multiplier
        [0.15, 1.5], # covariance front width
        ...
        #[0.5, 3.0], # sim_time
        [0.01, 2.0], # base obstacle scale
        [10, 40], # path dist scale
        [10, 40], # goal dist scale
        [10, 40] # path align scale
        #[0.3, 1.5] # inflation radius
        ]
        ...
        state = []
        # --Noé: try to remove the goal info
        ...
        for i in range(self.lidar_points):
            state = state +  [[0., 3.]] #[[0., 3.]] higher range?
