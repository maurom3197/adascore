...
class SAC(OffPolicyAgent):
    ...
    def __init__(
        ...
        self._setup_actor(state_shape, action_dim, state_2d_shape, actor_units, lr, max_action, min_action, network)
        self._setup_critic_q(state_shape, action_dim, state_2d_shape, critic_units, lr)
    ...
    def _setup_actor(self, state_shape, action_dim, state_2d_shape, actor_units, lr, max_action, min_action, network='mlp'):
        if network=='mlp':
            self.actor = GaussianActor(
                state_shape, action_dim, max_action, min_action, squash=True, units=actor_units)
        elif network=='conv':
            self.actor = ConvGaussianActor(
                state_shape, state_2d_shape, action_dim, max_action, min_action, squash=True, units=actor_units)
        self.actor_optimizer = tf.keras.optimizers.Adam(learning_rate=lr)
        #if self.log_level < 20:
        #    self.actor.model().summary()

    def _setup_critic_q(self, state_shape, action_dim, state_2d_shape, critic_units, lr, network='mlp'):
        if network=='mlp':
            ...
            self.qf1 = ConvMixCriticQ(state_shape, state_2d_shape, action_dim, critic_units=critic_units, name="qf1")
            if self.log_level < 20:
            	self.qf1.model().summary()
            self.qf2 = ConvMixCriticQ(state_shape, state_2d_shape, action_dim, critic_units=critic_units, name="qf2")
            self.qf1_target = ConvMixCriticQ(state_shape, state_2d_shape, action_dim, critic_units=critic_units, name="qf1_target")
            self.qf2_target = ConvMixCriticQ(state_shape, state_2d_shape, action_dim, critic_units=critic_units, name="qf2_target")
