...
from pic4rl.utils.env_utils import filter_people, process_people, normalize, normalize_angle
...
class SocialForceModel():
    ...
    def get_param(self, agents_config):
        configFilepath = os.path.join(
            get_package_share_directory("hunav_agent_manager"), 'config',
            agents_config)
    ...
    def get_people(self, robot_pose, robot_velocity):
        ...
        self.node.get_logger().debug("People callback")
        self.node.get_logger().debug("Number of people: "+str(len(msg.people)))
