...
class Pic4DWA:
	...
	def calc_heading_cost(self, trajectory, goal):
		...
		# Using distance from goal: better performances
		dx = goal[0] - trajectory[-1][0]
		dy = goal[1] - trajectory[-1][1]
		dist = np.hypot(dx,dy)
		if goal != self.previous_goal:
			...
			self.alfa = 1/dist
			print("============================\nNew goal! Alfa: ", self.alfa)
			print("goal: ", goal)
			print("pose: ", trajectory[-1][0], trajectory[-1][1])
			print("dist: ", dx, dy, dist)
			print("============================\n")
