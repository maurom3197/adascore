...
class Pic4DWA:
	...
	def __init__(self):
		...
		self.robot_width = 0.40  # [m] for collision check rectangle
		...
		self.Vs = [self.min_speed, self.max_speed,
	...
	def calc_dynamic_window(self, x):
		...
		dw = [max(self.Vs[0], Vd[0]), min(self.Vs[1], Vd[1]),
	...
	def calc_control_and_trajectory(self, x, dw, goal, ob):
		...
		for v in np.arange(dw[0], dw[1], self.v_resolution):
			for w in np.arange(dw[2], dw[3], self.yaw_rate_resolution):
				...
				# final_cost = heading_cost + ob_cost
