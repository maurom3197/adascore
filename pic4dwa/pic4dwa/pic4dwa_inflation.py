...
class Pic4DWA:
	...
	def calc_control_and_trajectory(self, x, dw, goal, ob):
		...
		for v in np.arange(dw[0], dw[1], self.v_resolution):
			for w in np.arange(dw[2], dw[3], self.yaw_rate_resolution):
				...
				dist, inflation    = self.calc_obstacle_cost(trajectory, ob)
				...
				final_cost = heading_cost + speed_cost + ob_cost + inflation
		...
		print("min_inflation: ", best_inflation)
	...
	def calc_obstacle_cost(self, trajectory, ob):
		...
		for i in range(len(r)):
			for j in r[i]:
				true_dist = j - self.robot_width/2
				if true_dist <= self.inflation_radius:
					inflation += (self.inflation_radius/true_dist -1)
