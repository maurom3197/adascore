...
class Pic4rlEnvironmentAdascore(Node):
    def __init__(self):
        ...
        # init goal publisher
        self.hunav_goal_pub = self.create_publisher(PoseStamped, "hunav_goal_pose", qos)
    ...
    def _step(self, nav_params=None, reset_step=False):
        ...
        if done:
            ...
            if (
                self.mode == "testing"
                and self.hunav_eval is not None
                and self.eval_running
            ):
        ...
        if self.navigator.isTaskComplete():
            ...
            if result == TaskResult.FAILED or result == TaskResult.CANCELED:
                ...
                if self.n_navigation_end == 50:
                    self.get_logger().info(
                        "Navigation aborted more than 50 times... pausing Nav till next episode."
                    )
                    self.get_logger().info(
                        f"Ep {'evaluate' if self.evaluate else self.episode+1}: 'Nav failed'"
                    )
                    logging.info(
                        f"Ep {'evaluate' if self.evaluate else self.episode+1}: Nav failed"
                    )
    ...
    def get_reward(self, local_goal_info, social_work, robot_velocity, event):
        ...
        ch = 0.4
        Rh = 1 - 2 * math.sqrt(
            math.fabs(local_goal_info[1] / math.pi)
        )  # heading reward v.2

        # Linear Velocity Reward
        cv = 0.8
        Rv = (
            robot_velocity[0] - self.max_lin_vel
        ) / self.max_lin_vel  # velocity reward

        # Obstacle Reward
        co = 2.0
        Ro = (
            self.min_obstacle_distance - self.lidar_distance
        ) / self.lidar_distance  # obstacle reward
    ...
    def reset(self, n_episode, tot_steps, evaluate=False):
        ...
        if self.mode == "testing":
            Path(os.path.join(self.logdir, "evaluator/")).mkdir(
                parents=True, exist_ok=True
            )
            evaluator_path = str(
                Path(os.path.join(self.logdir, "evaluator", "metrics"))
            )
            ...
            if self.hunav_eval is None:
                self.hunav_eval = subprocess.Popen(
                    f"ros2 launch hunav_evaluator hunav_evaluator_launch.py metrics_output_path:={evaluator_path} &",
                    shell=True,
            ...
            if self.bag_episode == 0:
                Path(os.path.join(self.logdir, "bags")).mkdir(
                    parents=True, exist_ok=True
                )
            self.bag_process = subprocess.Popen(
                f"ros2 bag record {topic_list} -o {self.logdir}/bags/episode_{self.bag_episode}",
                shell=True,
    ...
    def new_episode(self):
        ...
        if self.episode % 1 == 0.0 or self.mode == "testing":
    ...
    def respawn_agents(self, all=False):
        ...
        for agent in agents2reset:
            ...
            self.get_logger().debug(
                f"Respawning Agent at pose [x,y,yaw]: {[x, y, yaw]}"
            )
    ...
    def reset_navigator(self, index):
        ...
        self.navigator.waitUntilNav2Active(
            navigator="bt_navigator", localizer=localizer
        )
    ...
    def create_clients(
        ...
        self.set_entity_state_client = self.create_client(
            SetEntityState, "test/set_entity_state"
        )

        self.hunav_trigger_recording_client = self.create_client(
            Trigger, "hunav_trigger_recording"
        )
    ...
    def call_hunav_service(self, stop):
        ...
        while not self.hunav_trigger_recording_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("service not available, waiting again...")
            self.get_logger().info(
                f"Process: {self.hunav_eval} : is it None? {self.hunav_eval is None}"
            )
        self.get_logger().info(
            f"Sending request to {'stop' if stop else 'start'} recording..."
        )
