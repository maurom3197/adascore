...
class SocialForceModel:
    def __init__(self, node, agents_config):
    ...
    def get_param(self, agents_config):
        configFilepath = os.path.join(
            get_package_share_directory("adascore"),
            "config",
            "agents_envs",
            agents_config,
        )
    ...
    def instanciate_agents(
        ...
        for i in range(self.num_agents):
            agent = Agent()
            agent.agent_name = self.config_params["agents"][i]
    ...
    def get_people(self, robot_pose, robot_velocity):
        ...
        people_state, people_info_, min_people_distance = filter_people(
            self.node.k_people, distances, people_state_, people_info_
        )
    ...
    def computeSocialForce_on_agent(self, index):
        ...
        for i in range(len(self.agents)):
            ...
            diff = np.array(self.agents[i].position) - np.array(
                self.agents[index].position
            )
            diffDirection, diff_norm = normalize(diff)
            velDiff = np.array(self.agents[index].velocity) - np.array(
            ...
            theta = math.atan2(diffDirection[1], diffDirection[0]) - math.atan2(
            ...
            # Compute force
            forceVelocityAmount = -math.exp(
                -diff_norm / B - (self.agents[index].nPrime * B * theta) ** 2
            )

            forceAngleAmount = -np.sign(theta) * math.exp(
                -diff_norm / B - (self.agents[index].n * B * theta) ** 2
            )
            ...
            social_force = social_force + self.agents[index].forceFactorSocial * (
                forceVelocity + forceAngle
            )
    ...
    def computeSocialForce_by_robot(self, index):
        ...
        # Compute force
        forceVelocityAmount = -math.exp(
            -diff_norm / B - (agent.nPrime * B * theta) ** 2
        )

        forceAngleAmount = -np.sign(theta) * math.exp(
            -diff_norm / B - (agent.n * B * theta) ** 2
        )
